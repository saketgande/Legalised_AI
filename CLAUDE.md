...
| Commit | What |
|---|---|
| `7291580` | **Assistant → workspace (page-depth pass 1/N)** — rebuilds `/assistant` from a single-column empty chat box into a comprehensive, interconnected AI workspace. Two-column layout: a 240px **thread rail** (up to 25 conversations persisted to `localStorage`, "New conversation", and a "Jump into a matter" list that scopes the next question) + the conversation column. **Inline capability spawning** — a router classifies each question and spawns the right *real* tool inside the thread instead of routing to another page: compare/across → a live **Tabular** grid (`tabularDocuments`+`tabularRun`, §-linked cells); draft/rewrite/clause → a grounded **Editor** draft (`editorDraft`); review/recommend/next-steps → an **Agent** run (`agentRun`, streamed plan→steps→recommendation); else → the grounded **Assistant** answer with `[n]` citations. Each renders as an inline `RichCard` reusing the existing tab-grid / agent-step / diff-after atoms and links out to the full tool. Scope selector threads through every capability. No backend change — every call goes through the existing typed `api.ts` client and the four already-audited SSE engines. Verified live: grounded ask (8 sources), inline 6-doc compare grid, 3-step inline agent run, threads persist across reload; 0 page errors, clean build |
